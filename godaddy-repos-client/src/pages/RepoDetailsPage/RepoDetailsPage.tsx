...
import { fetchRepoByName, RepoItem } from '../../data/repoListDataProvider';
...
const RepoDetailsPage: React.FC = () => {
  ...
  const [repo, setRepo] = useState<RepoItem | undefined>(location.state?.repo);
  ...
  useEffect(() => {
    if (!repoName) {
      setLoading(false);
      setError('No repository name provided in the URL.');
    }
    ...
    if (!repo && repoName) {
      ...
      setLoading(true);
        try {
          const data = await fetchRepoByName(repoName);
          setRepo(data ?? undefined);
        } catch (error) {
          setRepo(undefined);
          setError(error?.toString() ?? 'Repository not found.');
        } finally {
          setLoading(false);
        }
      };
  ...
  return (<>
    {loading ? (
      ...
      <>
        <PageHeader title={`${repo?.name ?? repoName} Repository Details`} subtitle={message} onSubtitleClick={handleSubtitleClicked} />
        {error || !repo ? (
          <Toast type="error" message={error ?? 'Repository not found.'} />
        ) : (
